...
| **Human-in-the-loop** | The human gate is the action rail's out-of-band approval: `commit` only submits a real-world action (call, text, form), and a separate endpoint consumes a single-use credential bound to that action, so an agent cannot approve its own action. Direct clinical FHIR writes are **not** gated by a human today. They return HTTP 428 until an `X-Human-Confirmed` header is present, but the caller sets that header about itself. This is a known gap ([#214](https://github.com/aks129/HealthClawGuardrails/issues/214)), not a control; do not build on it. |
