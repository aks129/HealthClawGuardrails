...
- **Provably out-of-band human gate.** An agent can *propose* an action, but `commit` only *submits* it (HTTP 202). Execution happens only through a separate approval endpoint. That endpoint requires a single-use step-up credential and wins an expiry-guarded atomic claim. No code path lets the agent's own tools approve their own action. The spoofable `X-Human-Confirmed` header plays no part in the rail. It still gates direct clinical FHIR writes, a known gap (#214).
...
- **CareAgents: the hosted consumer experience.** A non-developer can spin up a guardrailed health agent in a minute at [careagents.cloud](https://careagents.cloud). You **sign in with your face** (a WebAuthn passkey; the biometric stays on-device), connect records, create an agent, chat, and run the forms rail. CareAgents' *only* data path is the HealthClaw guardrail layer's HTTP API. It adds experience, never policy, and stores no PHI. A connection is a pointer to a HealthClaw tenant. Shipped so far: a **pluggable connector marketplace** with Fasten, Apple Health via Open Wearables, sample records and honest "coming soon" tiles (#138/#140). **Surfaces** are the web, and iMessage where it's offered (#135–#137); Telegram is built but not open in the beta (#536). An **advisor registry** carries specialties ported from the archived SmartHealthConnect, as prompt blocks over the guarded tool set. **Versioned informed consent** is enforced server-side before any real-record connection. Claude-subscription auth uses `ANTHROPIC_OAUTH_TOKEN` (#145).
