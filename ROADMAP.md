...
- **CareAgents — the hosted consumer experience.** A non-developer spins up a guardrailed health agent in a minute at [careagents.cloud](https://careagents.cloud): **sign in with your face** (WebAuthn passkey — biometric stays on-device), connect records, create an agent, chat, run the forms rail. CareAgents' *only* data path is the HealthClaw guardrail layer's HTTP API — it adds experience, never policy, and stores no PHI (a connection is a pointer to a HealthClaw tenant). Shipped so far: a **pluggable connector marketplace** (Fasten, Apple Health via Open Wearables, sample records, honest "coming soon" tiles — #138/#140), **surfaces** (web, and iMessage where it's offered; #135–#137), an **advisor registry** (specialties ported from the now-archived SmartHealthConnect, prompt-blocks over the guarded tool set), **versioned informed consent** enforced server-side before any real-record connection, and Claude-subscription auth (`ANTHROPIC_OAUTH_TOKEN`, #145). Telegram is built but not open in the beta (#536).
