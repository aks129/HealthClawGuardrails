...
> **Superseded 2026-09-24.** The owner ruled that 2.0 ships as the
> synthetic-beta release from September's `main`. The alpha, beta and
> November milestones below become 2.x work. The new definition and the
> current ratchet numbers are in
> [the playbook's §5](2026-08-05-healthclaw-2.0-playbook.md#5-definition-of-done-for-20).
> The plan is kept as written.
...
## 8. RETRACTED — the finding that was not one

**This section claimed `$conformance` awards Grade A on an empty response. It
does not, and #443 is closed as invalid.**

`Check` is `(name, passed, detail)`. `detail` is the static explanation of
what a failure *would* mean, attached whether or not the check fails. The
`"passed": true` quoted here meant `_is_resource(body, "Patient", pid)`
returned True — the record came back, the five absence checks below it were
meaningful, and the A was earned. A warning label was read as a confession.

`r6/conformance/probes.py:471` is a deliberate two-sided guard added in #213,
with a second at `:497` for the search path. It is the pattern this document
elsewhere argues for, already implemented. The defect was filed against the
best existing instance of the thing being proposed.

The section is struck rather than deleted because the error is more
instructive than the claim was: it is Law 3 of
`docs/2026-08-06-two-generators-three-laws.md` broken in the act of writing
the document that proposes it — a verifier asserted broken without ever being
seen to fail. That document's §8 carries the full account.

Nothing else in this retro depends on this section.
