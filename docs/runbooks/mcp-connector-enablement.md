...
|---|---|---|
| Authorization-server conformance (302, `aud`, client auth, root discovery) | #669, merged | **Yes.** The Flask app auto-deploys from `main`. |
| Introspection and the read-only mint | #695, merged | Yes, inert: introspection refuses everybody until its client credential is set |
| Consent handoff, audit, revoke | #697, merged | Yes, inert: without `CAREAGENTS_CONSENT_URL` the MCP audience keeps the demo policy |
| Rotating refresh tokens | #698, merged | Yes, live: every code grant returns a refresh token from then on |
| CareAgents consent page | #670, merged | No: CareAgents is a manual deploy (`docs/runbooks/careagents-durable-worker.md`) |
| MCP server OAuth path | #671, merged | No: the MCP server is a manual deploy, and off until `MCP_OAUTH_ENABLED=true` |
| Walkthrough | #672, merged | n/a |

Everything in this table is on `main` as of 12 September 2026. What is
**not** done is every step below: DNS, variables, the two manual deploys,
and the enable. The connector cannot work in production until they are.
